...
int32 Lan_TcpServerHandler(pgcontext pgc)
{
    ...
    int addrLen = sizeof(struct sockaddr_t);
     static uint8 seltcpFlag = 0;
    
    if(pgc->ls.tcpServerFd < 0)
    {
        return RET_FAILED;
    }
    if(FD_ISSET(pgc->ls.tcpServerFd, &(pgc->rtinfo.readfd))&&(getselFlag()))
    {
        ...
        newfd = Socket_accept(pgc->ls.tcpServerFd, &addr, &addrLen);
         ...
         if( newfd<0 && (seltcpFlag++ == 0) )
            {
                GAgent_Printf( GAGENT_ERROR,"tcp First time, ignore it!!!");
                return RET_FAILED;
            }else if( newfd<0)
            {
            GAgent_Printf( GAGENT_ERROR,"Need to Restart Lan_TcpServer, newfd=%d ", newfd);
            //Lan_CreateTCPServer(&(pgc->ls.tcpServerFd), GAGENT_TCP_SERVER_PORT); //Nik.chen
            return RET_FAILED;
           }
                ...
                ppacket prxBuf,/* ppacket ptxBuf,*/ int32 buflen)
{
	...
	#if 0
GAgent_Printf(GAGENT_DEBUG, "LAN tcp data Lenth = %d\n\r",recDataLen);
GAgent_Printf( GAGENT_DEBUG,"LAN+++++++++++++++++++++++++++++++\r\n");
for( int i=0;i<recDataLen;i++ )
    GAgent_Printf( GAGENT_DUMP," %02X",prxBuf->phead[i] );
GAgent_Printf( GAGENT_DEBUG,"LAN++++++++++++++++++++++++++++++++\r\n");
	#endif
...
int32 Lan_dispatchTCPData(pgcontext pgc, ppacket prxBuf,/* ppacket ptxBuf,*/ int32 clientIndex)
{
    ...
    switch (cmd)
    {
        case GAGENT_LAN_CMD_BINDING:
            ...
            GAgent_Printf(GAGENT_WARNING,"GAGENT_LAN_CMD_BINDING!!!");
            break;
        case GAGENT_LAN_CMD_LOGIN:
             GAgent_Printf(GAGENT_WARNING,"GAGENT_LAN_CMD_LOGIN!!!");
            Lan_handleLogin( pgc, prxBuf,clientIndex );
            break;
        case GAGENT_LAN_CMD_CTL_93:
            for(ii =0; ii<bytesOfLen; ii++)
            {
                 GAgent_Printf(GAGENT_WARNING,"playload[%d]=%d(%02x)", ii, *(uint8 *)(prxBuf->phead + LAN_PROTOCOL_HEAD_LEN + LAN_PROTOCOL_FLAG_LEN
                        + bytesOfLen+ii+1+4), *(uint8 *)(prxBuf->phead + LAN_PROTOCOL_HEAD_LEN + LAN_PROTOCOL_FLAG_LEN
                        + bytesOfLen+ii+1+4));
            }
        ...
        case GAGENT_LAN_CMD_TRANSMIT:
             for(ii =0; ii<bytesOfLen+1; ii++)
            {
                 GAgent_Printf(GAGENT_WARNING,"playload[%d]=%d(%02x)", ii, *(uint8 *)(prxBuf->phead + LAN_PROTOCOL_HEAD_LEN + LAN_PROTOCOL_FLAG_LEN
                        + bytesOfLen+ii+1), *(uint8 *)(prxBuf->phead + LAN_PROTOCOL_HEAD_LEN + LAN_PROTOCOL_FLAG_LEN
                        + bytesOfLen+ii+1));
            }
            prxBuf->type = SetPacketType( prxBuf->type,LAN_TCP_DATA_IN,1 );
            ParsePacket(prxBuf);
...
{
    GAgent_Printf( GAGENT_DEBUG,"Lan_CreateTCPServer." );
    *pFd = GAgent_CreateTcpServer( tcp_port );
    GAgent_Printf( GAGENT_DEBUG,"pFd=%d.", (*pFd));
}
