...
****************************************************************/
int32 GAgent_CreateWebConfigServer( uint16 tcp_port ) //webconfig
{   
...
****************************************************************/
void GAgent_DoTcpWebConfig( pgcontext pgc ) //webconfig
{
    ...
    if( (GAgentStatus&WIFI_MODE_AP)!= WIFI_MODE_AP )
    {
        ...
        GAgent_Printf( GAGENT_DEBUG,"Creat Tcp Web Server, not AP mode, return!!" );
        return ;
    }
 
    if( (GAgentStatus&WIFI_MODE_ONBOARDING)!= WIFI_MODE_ONBOARDING )
    {
        GAgent_Printf( GAGENT_DEBUG,"Creat Tcp Web Server, not ONBOARDING, return!!" );
        return ;
    }
    #endif
    ...
    GAgent_Printf( GAGENT_DEBUG,"tcpWebConfigFd=%d.", pgc->ls.tcpWebConfigFd);
    if( !(FD_ISSET(pgc->ls.tcpWebConfigFd, &(pgc->rtinfo.readfd))&&(getselFlag())) )
        return ;
    ...
    GAgent_Printf( GAGENT_DEBUG,"newfd=%d.", newfd);
