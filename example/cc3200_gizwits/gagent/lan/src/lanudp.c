...
void Lan_udpDataHandle(pgcontext pgc, ppacket prxBuf, int32 len)
{
    ...
    int32 recLen = 0;
    static uint8 seludpFlag = 0;

    if( pgc->ls.udpServerFd>0 )
    {
     //GAgent_Printf( GAGENT_DEBUG,"Lan_udpDataHandle enter, isset=%d, errno=%d!!!", (FD_ISSET(pgc->ls.udpServerFd, &(pgc->rtinfo.readfd))), errno);
     if(FD_ISSET(pgc->ls.udpServerFd, &(pgc->rtinfo.readfd))&&(getselFlag()))
    {
        ...
        recLen = Socket_recvfrom(pgc->ls.udpServerFd, prxBuf->phead, len,
            &addr, &addrLen);
        GAgent_Printf( GAGENT_CRITICAL,"UDP bytes received = %d, errno=%d",recLen, errno );
            if( recLen<=0 && (seludpFlag++ == 0) )
            {
                GAgent_Printf( GAGENT_ERROR,"udp First time, ignore it!!!");
                return ;
            }else if( recLen<=0)
            {
                GAgent_Printf( GAGENT_ERROR,"Need to Restart Lan udp Server!!!");
                //Lan_CreateUDPServer( &(pgc->ls.udpServerFd), LAN_UDP_SERVER_PORT ); //Nik.chen
                return ;
            }
            ...
            ppacket prxBuf, int32 recLen)
{
    ...
    cmd = ntohs(cmd);
    GAgent_Printf(GAGENT_WARNING, " cmd=%d \n\r", cmd);
    switch(cmd)
    {
        ...
        case GAGENT_LAN_CMD_ONBOARDING:
        {
            ...
            {
                GAgent_Printf(GAGENT_ERROR, "NOT in WIFI_MODE_AP return!!!");
                return ;
            }
            if( (TempWiFiStatus&WIFI_MODE_ONBOARDING)!= WIFI_MODE_ONBOARDING ) 
            {
                GAgent_Printf(GAGENT_ERROR, "NOT in WIFI_MODE_ONBOARDING return!!!");
                return ; //Nik.chen mark fot test
            }
...
static int32 Lan_udpOnBoarding(pgcontext pgc, uint8 *buf)
{
    ...
    GAgent_Printf( GAGENT_INFO,"Lan_udpOnBoarding ssid=%s, key=%s\n ", pgc->gc.wifi_ssid,pgc->gc.wifi_key);
...

{
    ...
    *(uint16 *)(ptxBuf + offset) = htons(GAGENT_LAN_CMD_REPLY_ONBOARDING);
    ...
    if(ret != len)
    {
        GAgent_Printf(GAGENT_DEBUG,"send onboarding response fail,len:%d.ret:0x%x",len, ret);
    }else
          GAgent_Printf(GAGENT_DEBUG, "LAN_onBoardingAck successfully!!!");
