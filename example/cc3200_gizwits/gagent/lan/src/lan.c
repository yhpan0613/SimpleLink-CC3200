...
static void Lan_Upload_AllApp(pgcontext pgc, ppacket pTxBuf)
{
    ...
    for(i = 0; i < LAN_TCPCLIENT_MAX; i++)
    {
        ...
        {
        	GAgent_Printf( GAGENT_DEBUG,"Lan_Upload_AllApp i=%d \r\n", i );
...
void Lan_sendTcpData(pgcontext pgc, int32 fd, uint16 cmd, int32 sn, ppacket pTxBuf)
{
    ...
    *(uint16 *)(pTxBuf->phead + offset) = htons(cmd);
    ...
    switch(cmd)
    {
		...
		//for(i=0;i<10;i++)
		//GAgent_Printf( GAGENT_DEBUG,"i=%d,  data[%d]=%02X\r\n", i, i, pTxBuf->phead[i] );


    if(fd >= 0)
    {
         GAgent_Printf( GAGENT_DEBUG,"Lan_sendTcpData send 1 socket\r\n");
        send(fd, pTxBuf->phead, pTxBuf->pend - pTxBuf->phead, 0);
    }
    else
    {
         GAgent_Printf( GAGENT_DEBUG,"Lan_sendTcpData send all socket\r\n");
...
uint32 GAgent_Lan_Handle(pgcontext pgc, ppacket prxBuf,int32 len)
{
    ...
    GAgent3rdLan_Handle( pgc );
     GAgent_DoTcpWebConfig( pgc ); //Nik.chen for test
        for(i = 0;i < LAN_TCPCLIENT_MAX; i++)
        {
            fd = pgc->ls.tcpClient[i].fd;
            if(fd < 0)
                continue;
            if(FD_ISSET(fd, &(pgc->rtinfo.readfd))&&(getselFlag()))
            {
...
void RecreatLanSocket(pgcontext pgc)
{
   ...
   if(NULL == pgc)
    ...
    if(INVALID_SOCKET != pgc->ls.udpServerFd)
    {
        close(pgc->ls.udpServerFd);
        pgc->ls.udpServerFd = INVALID_SOCKET;
        GAgent_Printf( GAGENT_INFO,"close udp socket! ..." );
    }
     if(INVALID_SOCKET != pgc->ls.tcpServerFd)
    {
        close(pgc->ls.tcpServerFd);
        pgc->ls.tcpServerFd = INVALID_SOCKET;
        GAgent_Printf( GAGENT_INFO,"close tcp socket! ..." );
    }

    DestroyUDPBroadCastServer(pgc);

     if( pgc->ls.tcpWebConfigFd != INVALID_SOCKET )
        {
            close( pgc->ls.tcpWebConfigFd );
            pgc->ls.tcpWebConfigFd = INVALID_SOCKET;
            GAgent_Printf( GAGENT_INFO,"tcpWebConfigFd! ..." );
        }
