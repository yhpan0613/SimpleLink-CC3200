...
void GAgent_SOC_GetInfo( pgcontext pgc )
{
    ...
    int8 *mcu_hard_ver="00CC3200";
    int8 *mcu_soft_ver="04020002";
    //4b373b65faab4633af7de4411fc516ce yt 
    //7d99f359f5954e38a48eaf159c44f1a1 demo 
    //637c0ee1c0a4411c8d85277324ae7523 iot
# if (GAGENT_RELEASE == 1)
    int8 *mcu_product_key=PRODUCT_KEY;
# else 
    int8 *mcu_product_key=PRODUCT_KEY;//
#endif
    ...
    strcpy( (char*)pgc->mcu.protocol_ver,mcu_p0_ver );
    ...
    if( strcmp( (int8 *)pgc->mcu.product_key,pgc->gc.old_productkey )!=0 )
    {
        ...
        GAgent_Printf( GAGENT_INFO,"It's a new product_key:%s.",pgc->mcu.product_key );
        GAgent_Printf( GAGENT_INFO,"GAgent go to  update info...");
    }
...
static int Local_CheckAck(pgcontext pgc, int32 cmd, int32 sn)
{
    ...
    GAgent_Printf(GAGENT_DEBUG, "cmdTx=%d, snTx=%d, sn=%d", cmdTx, snTx, sn);
...
int32 GAgent_LocalDataWriteP0( pgcontext pgc,int32 fd,ppacket pTxBuf,uint8 cmd )
{
    ...
    *( pTxBuf->pend )  = GAgent_SetCheckSum(pTxBuf->phead, (pTxBuf->pend)-(pTxBuf->phead) );
    ...
    do
    {
        //GAgent_Local_WaitDataReady(pgc, 0, 10); //Nik.chen mark
        
        nowTime = GAgent_GetDevTime_MS();
        //GAgent_Printf(GAGENT_INFO,"Time  ms %d, resendTime=%d",nowTime, resendTime );
        #if 0
        mdelay(800000);
        ...
        if(dTime >= 30) //Nik.chen MCU_ACK_TIME_MS ->
        {
           if(resendTime!=1)
                GAgent_Local_Handle(pgc, pgc->rtinfo.Rxbuf, 0);
           else
            {
                if(dTime >= 100)  //Nik.chen 200 -> ?
                    GAgent_Local_Handle(pgc, pgc->rtinfo.Rxbuf, 0);
            }
        }
        #endif
       ...
       // GAgent_Printf(GAGENT_INFO,"nowTime:%d, preTime:%d",nowTime, preTime );

        dTime = abs(nowTime - preTime);
        if(dTime >= MCU_ACK_TIME_MS ) //Nik.chen MCU_ACK_TIME_MS ->
        {
            ...
            GAgent_Printf( GAGENT_INFO,"resend_time=%d, Time ms %d",resendTime, GAgent_GetDevTime_MS );
           // if( resendTime>=2 )
            Local_SendData( fd, pTxBuf->phead, sendLen );
            pgc->mcu.isBusy = 1;
            preTime=GAgent_GetDevTime_MS();
...
void GAgent_LocalInit( pgcontext pgc )
{
    ...
    Local_HalInit( pgc );
    GAgent_RegisterReceiveDataHook( GAgent_Local_RecAll );  //Nik.chen mark for test
    GAgent_RegisterSendDataHook( serial_write );
    #if 1 //Nik.chen mark fot soc test
     Local_GetInfo( pgc );
    #else
        ...
        switch( cmd )
        {
		  ...
		  GAgent_ExitTest( pgc ); 
            ...
            case MCU_INFO_CMD_ACK:
                ...
                break;
            case MCU_CTRL_CMD_ACK:
		   GAgent_Printf(GAGENT_DEBUG, "MCU_CTRL_CMD_ACK");
                if(RET_SUCCESS == Local_CheckAck(pgc, cmd, sn))
                {
                if(pgc->rtinfo.file.using == 1)
                {
                    ret = 0;
			GAgent_Printf(GAGENT_DEBUG, "file_using break");
                    break;
                }
...
void GAgent_Local_Handle( pgcontext pgc,ppacket Rxbuf,int32 length )
{
    ...
    mcudataLen = GAgent_Local_RecAll(pgc);
    ...
    do
    {
        ...
        #if 0
        mcudataLen = Rxbuf->phead[2];
        UART_PRINT("localDataLen=%d, mcudataLen=%d, local buf: \n", localDataLen, mcudataLen);
         for(ii = 0; ii < (mcudataLen + 2); ii++)
        {
            UART_PRINT("%02x ", Rxbuf->phead[ii]);
        }
